...
const DEFAULT_USER_ID = 'openmaic-problem-video-ui';
export const DEFAULT_CHAT_USER_ID = 'openmaic-default-user';
  ...
  try {
    ...
    const hint =
      /connection error|econnrefused|fetch failed|network|refused|timed out|timeout/i.test(message)
        ? ' 请确认已在运行 AnotherMe2 网关（如 pnpm dev:gateway），且 OpenMAIC 的 ANOTHERME2_GATEWAY_BASE_URL 与网关监听地址一致；若使用 pnpm dev:all 复用已有 Next 进程，请核对 .env.local 中的网关 URL。'
        : '';
    throw new AnotherMe2GatewayError(
