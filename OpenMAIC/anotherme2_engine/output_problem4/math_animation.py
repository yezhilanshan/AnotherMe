...
class Abp60PqPbqPqcApb150Problem(Scene):
    def construct(self):
        ...
        def _safe_add_sound(path, time_offset=0.0):
            ...
            try:
                ...
                return
            try:
                self.add_sound(path, time_offset=time_offset)
            except Exception:
                pass

        # Step 1: 题目分析
        _safe_add_sound(r'output_problem4/audio/narration_001.mp3', time_offset=0.00)
        ...
        # Step 2: 构造旋转
        _safe_add_sound(r'output_problem4/audio/narration_002.mp3', time_offset=0.00)
        if len(current_formula_group) > 0:
            self.play(FadeOut(current_formula_group), run_time=0.20)
        current_formula_group = VGroup()
        step_formula_group = VGroup()
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('构造：旋转△ABP 60°', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        highlight_anims.append(points['Q'].animate.set_color(YELLOW))
        highlight_anims.append(objects['poly_ABP'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_PQ'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_QB'].animate.set_color(YELLOW))
        self.play(*highlight_anims, run_time=0.80)
        transform_anims = []
        transform_anims.append(points['B'].animate.scale(1.05).set_color(ORANGE))
        transform_anims.append(points['Q'].animate.scale(1.05).set_color(ORANGE))
        transform_anims.append(objects['poly_ABP'].animate.scale(1.05).set_color(ORANGE))
        transform_anims.append(lines['seg_PQ'].animate.set_color(ORANGE))
        transform_anims.append(lines['seg_QB'].animate.set_color(ORANGE))
        self.play(*transform_anims, run_time=0.90)
        restore_anims = []
        restore_anims.append(points['B'].animate.set_color(WHITE))
        restore_anims.append(points['Q'].animate.set_color(WHITE))
        restore_anims.append(objects['poly_ABP'].animate.set_color(BLUE))
        restore_anims.append(lines['seg_PQ'].animate.set_color(BLUE_E).set_stroke(width=3))
        restore_anims.append(lines['seg_QB'].animate.set_color(BLUE_E).set_stroke(width=3))
        self.play(*restore_anims, run_time=0.40)
        self.wait(5.09)

        # Step 3: 正三角形判定
        _safe_add_sound(r'output_problem4/audio/narration_003.mp3', time_offset=0.00)
        if len(current_formula_group) > 0:
            self.play(FadeOut(current_formula_group), run_time=0.20)
        current_formula_group = VGroup()
        step_formula_group = VGroup()
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('\\angle PBQ = 60^{\\circ}', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('△PBQ 是正三角形', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        current_formula_group = step_formula_group
        highlight_anims = []
        highlight_anims.append(objects['poly_PBQ'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_BP'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_QB'].animate.set_color(YELLOW))
        self.play(*highlight_anims, run_time=0.80)
        transform_anims = []
        transform_anims.append(objects['poly_PBQ'].animate.scale(1.05).set_color(ORANGE))
        transform_anims.append(lines['seg_BP'].animate.set_color(ORANGE))
        transform_anims.append(lines['seg_QB'].animate.set_color(ORANGE))
        self.play(*transform_anims, run_time=0.90)
        restore_anims = []
        restore_anims.append(objects['poly_PBQ'].animate.set_color(BLUE))
        restore_anims.append(lines['seg_BP'].animate.set_color(BLUE_E).set_stroke(width=3))
        restore_anims.append(lines['seg_QB'].animate.set_color(BLUE_E).set_stroke(width=3))
        self.play(*restore_anims, run_time=0.40)
        self.wait(5.88)

        # Step 4: 直角三角形性质
        _safe_add_sound(r'output_problem4/audio/narration_004.mp3', time_offset=0.00)
        if len(current_formula_group) > 0:
            self.play(FadeOut(current_formula_group), run_time=0.20)
        current_formula_group = VGroup()
        step_formula_group = VGroup()
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('\\angle PQC = 90^{\\circ}', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('△PQC 是直角三角形', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        current_formula_group = step_formula_group
        highlight_anims = []
        highlight_anims.append(objects['poly_PQC'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_PQ'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_QC'].animate.set_color(YELLOW))
        self.play(*highlight_anims, run_time=0.80)
        self.wait(4.40)

        # Step 5: 求解结论
        self.add_sound(r'output_problem4/audio/narration_005.mp3', time_offset=0.00)
        if len(current_formula_group) > 0:
            self.play(FadeOut(current_formula_group), run_time=0.20)
        current_formula_group = VGroup()
        step_formula_group = VGroup()
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('\\angle APB = \\angle CQB', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('= 60^{\\circ} + 90^{\\circ} = 150^{\\circ}', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        transform_anims = []
        transform_anims.append(objects['poly_PBQ'].animate.scale(1.05).set_color(ORANGE))
        transform_anims.append(objects['poly_PQC'].animate.scale(1.05).set_color(ORANGE))
        self.play(*transform_anims, run_time=0.90)
        restore_anims = []
        restore_anims.append(objects['poly_PBQ'].animate.set_color(BLUE))
        restore_anims.append(objects['poly_PQC'].animate.set_color(BLUE))
        self.play(*restore_anims, run_time=0.40)
        self.wait(8.07)
