...
class Scene132021AbODCAcbAcBe1AE2Bc3Be5CeProblem(Scene):
    def construct(self):
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('\\angle ABC = 2\\angle A', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('∠A = ∠ABE ⇒ ∠EBC = ∠ A', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('△ABC ∽ △BCE', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('∴ ∠A = ∠E', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        highlight_anims = []
        highlight_anims.append(points['A'].animate.set_color(YELLOW))
        highlight_anims.append(points['E'].animate.set_color(YELLOW))
        highlight_anims.append(objects['poly_ABC'].animate.set_color(YELLOW))
        highlight_anims.append(objects['poly_BCE'].animate.set_color(YELLOW))
        self.play(*highlight_anims, run_time=0.80)
        self.wait(14.22)
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('BC/CE = AC/BE = AB/BC', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        current_formula_group = step_formula_group
        highlight_anims = []
        highlight_anims.append(objects['arc_AB'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_AB'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_AC'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_BC'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_BE'].animate.set_color(YELLOW))
        highlight_anims.append(lines['seg_CE'].animate.set_color(YELLOW))
        self.play(*highlight_anims, run_time=0.80)
        self.wait(13.83)
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('CE = 9/5', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
