...
class ParallelLinesPropertyAndCriterion(Scene):
    def construct(self):
        l1 = Line(LEFT * 5 + UP * 1.5, RIGHT * 5 + UP * 1.5)
        l2 = Line(LEFT * 5 + DOWN * 1.5, RIGHT * 5 + DOWN * 1.5)
        t = Line(LEFT * 2.5 + DOWN * 3, RIGHT * 1 + UP * 3)
        ...
        angle1, label1 = make_angle_mark(l1.get_right(), p1, t.get_start(), radius=0.45, color=BLUE, label_text=r"\alpha")
        angle2, label2 = make_angle_mark(t.get_end(), p2, l2.get_left(), radius=0.45, color=BLUE, label_text=r"\alpha")

        self.play(Create(angle1), Write(label1))
        self.play(Create(angle2), Write(label2))
        self.play(Indicate(angle1), Indicate(angle2))

        parallel_mark1 = MathTex(r"\parallel").move_to(LEFT * 3.8 + UP * 1.8)
        parallel_mark2 = MathTex(r"\parallel").move_to(LEFT * 3.8 + DOWN * 1.2)
