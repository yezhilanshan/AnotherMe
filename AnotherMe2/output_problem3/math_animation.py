...
class PAbcdPbaPdaPabPcbProblem(Scene):
    def construct(self):
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('PQ // BC 且 PQ = BC', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('\\angle PCB = \\angle PQB', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        if len(temp_labels) > 0:
            ...
            self.play(FadeOut(temp_labels), run_time=0.40)
        restore_anims = []
        restore_anims.append(lines['seg_BC'].animate.set_color(BLUE_E).set_stroke(width=3))
        self.play(*restore_anims, run_time=0.40)
        self.wait(11.93)
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('\\angle PDA = \\angle QCD', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('P、D、C、Q 四点共圆', font_size=28, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('\\angle PAB = \\angle QDC = \\angle QPC = \\angle PCB', font_size=28, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        restore_anims.append(points['C'].animate.set_color(WHITE))
        restore_anims.append(points['P'].animate.set_color(WHITE))
        restore_anims.append(lines['seg_AB'].animate.set_color(BLUE_E).set_stroke(width=3))
        self.play(*restore_anims, run_time=0.40)
        self.wait(9.98)
