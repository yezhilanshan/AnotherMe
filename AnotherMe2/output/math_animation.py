...
class Scene1RtAbcC90CircBc6CmAc8CmBcdBdCAbCAdcCm22AbcC90CircDAcBdBcdBd(Scene):
    def construct(self):
        ...
        self.play(*highlight_anims, run_time=1.8)
        transform_anims = []
        transform_anims.append(points['C'].animate.scale(1.05).set_color(ORANGE))
        self.play(*transform_anims, run_time=1.9)
        temp_labels = VGroup()
        temp_labels.add(Text('C', font_size=24, color=GREEN).next_to(points['C'], UP * 0.25))
        if len(temp_labels) > 0:
            self.play(FadeIn(temp_labels), run_time=1.6)
            self.play(FadeOut(temp_labels), run_time=1.4)
        if len(current_formula_group) > 0:
            self.play(FadeOut(current_formula_group), run_time=1.2)
        current_formula_group = VGroup()
        ...
        self.play(*restore_anims, run_time=1.4)
        self.wait(16.84)

        # Step 2: 计算斜边AB长度
        self.add_sound(r'output/audio/narration_002.mp3', time_offset=21.14)
        highlight_anims = []
        ...
        self.play(*highlight_anims, run_time=1.8)
        temp_labels = VGroup()
        temp_labels.add(Text('A', font_size=24, color=GREEN).next_to(points['A'], UP * 0.25))
        temp_labels.add(Text('B', font_size=24, color=GREEN).next_to(points['B'], UP * 0.25))
        temp_labels.add(Text('C', font_size=24, color=GREEN).next_to(points['C'], UP * 0.25))
        if len(temp_labels) > 0:
            self.play(FadeIn(temp_labels), run_time=1.6)
            self.play(FadeOut(temp_labels), run_time=1.4)
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = MathTex('AB=\\sqrt{AC^2+BC^2}=\\sqrt{8^2+6^2}=10\\,\\mathrm{cm}', font_size=30, color=YELLOW)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        # Step 3: 利用折叠性质求AC\'长度
        self.add_sound(r'output/audio/narration_003.mp3', time_offset=33.94)
        highlight_anims = []
        highlight_anims.append(points['B'].animate.set_color(YELLOW))
        highlight_anims.append(points['C'].animate.set_color(YELLOW))
        highlight_anims.append(points['A'].animate.set_color(YELLOW))
        ...
        transform_anims.append(points['C'].animate.scale(1.05).set_color(ORANGE))
        transform_anims.append(points['A'].animate.scale(1.05).set_color(ORANGE))
        self.play(*transform_anims, run_time=1.9)
        temp_labels = VGroup()
        temp_labels.add(Text('B', font_size=24, color=GREEN).next_to(points['B'], UP * 0.25))
        temp_labels.add(Text('C', font_size=24, color=GREEN).next_to(points['C'], UP * 0.25))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('折叠性质：BC\'=BC=6cm，AC\'= AB-BC\'=4cm', font_size=22, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        restore_anims.append(points['C'].animate.set_color(WHITE))
        restore_anims.append(points['A'].animate.set_color(WHITE))
        self.play(*restore_anims, run_time=1.4)
        self.wait(8.54)

        # Step 4: 设未知数列勾股定理方程
        self.add_sound(r'output/audio/narration_004.mp3', time_offset=46.78)
        highlight_anims = []
        highlight_anims.append(points['C'].animate.set_color(YELLOW))
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('设CD=x，则C\'D=x，AD=8-x', font_size=22, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
        ...
        # Step 5: 解方程计算三角形面积
        self.add_sound(r'output/audio/narration_005.mp3', time_offset=50.14)
        highlight_anims = []
        highlight_anims.append(points['C'].animate.set_color(YELLOW))
        highlight_anims.append(points['D'].animate.set_color(YELLOW))
        self.play(*highlight_anims, run_time=1.8)
        ...
        max_width = max(block_nw * config.frame_width - 0.30, 1.2)
        max_height = max(block_nh * config.frame_height - 0.10, 0.45)
        formula_obj = Text('解得x=3，S△ADC\'=1/2×4×3 =6cm²', font_size=22, color=YELLOW, line_spacing=0.85)
        formula_obj.scale_to_fit_width(min(formula_obj.width, max_width))
        if formula_obj.height > max_height:
            formula_obj.scale_to_fit_height(max_height)
        formula_obj.move_to(np.array([block_x, block_y, 0]))
